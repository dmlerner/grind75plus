...
class Board:
    ...
    def __init__(self, leetcode):
        ...
        self.board = {}
        self.cell_options_by_value = {value: set(CELLS) for value in INDICES}
    ...
    def solve(self):
        ...
        if not self.unset_cells:
            return True
        for cell in CELLS:
            if cell in self.unset_cells:
                break

        # options = self.get_options(cell)
        for option in self.get_options(cell):
        ...
        # for cell in tuple(sorted(self.unset_cells)):
        #    options = self.get_options(cell)
        #    if not options:
        #        return False
