...
def get_first_free_interval_during(schedule, interval):
    first_free_interval_at_or_after = get_first_free_interval_at_or_after(schedule, interval.start)
...
def _get_free_interval(schedules, candidate):
    for schedule in schedules:
        ...
        if schedule_free_interval is None:
            return False, Interval(get_first_free_interval_at_or_after(schedule, candidate.end).start, float('inf'))
...
assert get_first_free_interval_at_or_after(to_intervals([[1, 2], [5, 7]]), 1.5) == Interval(2, 5)
assert get_first_free_interval_at_or_after(to_intervals([[1, 2], [5, 7]]), -3) == Interval(float('-inf'), 1)

assert get_first_free_interval_during(to_intervals([[1, 2], [5, 7]]), to_interval([-10, 10])) == Interval(-10, 1)
assert get_first_free_interval_during(to_intervals([[1, 2], [5, 7]]), to_interval([3, 6])) == Interval(3, 5)
assert get_first_free_interval_during(to_intervals([[1, 2], [5, 7]]), to_interval([5, 6])) is None
