...
def max_path_sum(t):
    ...
    positive_child_max_path_one_sided  = tuple(filter(lambda x: x > 0, child_max_path_one_sided))
    child_max_path_sum = tuple(map(max_path_sum, children))

    return max(t.val, max(child_max_path_sum), t.val + sum(positive_child_max_path_one_sided))
